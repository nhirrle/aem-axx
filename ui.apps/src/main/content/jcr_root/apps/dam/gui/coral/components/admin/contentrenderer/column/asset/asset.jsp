...
%><%@include file="/libs/dam/gui/coral/components/admin/contentrenderer/column/common/common.jsp"%><%
    ...
    String thumbnailUrl = request.getContextPath() + requestPrefix
            + getThumbnailUrl(asset, 48, showOriginalIfNoRenditionAvailable, showOriginalForGifImages, sling)
...
//Override default thumbnail for set when there is manual thumbnail defined
    if (dmSetManualThumbnailAsset != null) {
        thumbnailUrl = request.getContextPath() + requestPrefix
                + getThumbnailUrl(asset, 1280, showOriginalIfNoRenditionAvailable, showOriginalForGifImages, sling)
                + "?ch_ck=" + ck + requestSuffix;
    }
...
%><coral-columnview-item <%= attrs.build() %>>
    ...
    <coral-columnview-item-content>
        ...
        %>
        <a class="axx-ribbon <%= !statusAsString.isEmpty() ? "axx-ribbon--" + statusAsString : "" %>" title="<%= statusAsString.isEmpty() ? "Never published" : org.apache.commons.lang.StringUtils.capitalize(statusAsString)%>"></a>
